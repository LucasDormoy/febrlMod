...
baseName=''.join(re.findall('(\w*)\.',output_file))
origName=baseName+'_orig.csv'
dupName=baseName+'_dup.csv'

try:
  f_out = open(output_file,'w')
except:
  print 'Error: Can not write to output file "%s".\n\
%s and %s not written either.' % (output_file, origName,dupName)
  sys.exit()
...
for rec_id in all_rec_ids:
  ...
  out_line = ''
  for field_name in field_name_list:
    out_line = out_line + rec_dict.get(field_name, missing_value).upper() + ','
...
# Reload ouptut file and split into two files -- inefficient, but easy

# load file, extract ID, and get index for originals
data=pd.read_csv(output_file)
data['ID']=data.iloc[:,0].str.extract('-([0-9]+)-')
orgIndex=data.iloc[:,0].str.contains(r'ORG|org')

# remove rec-##-org/dup column, and reorder columns so ID is first
data=data.iloc[:,1:]
cols=data.columns.tolist()
cols=cols[-1:]+cols[:-1]
data=data[cols]

# split into original and duplicates and write to files
original=data.iloc[orgIndex.index[orgIndex==True],:]
duplicates=data.iloc[orgIndex.index[orgIndex==False],:]


baseName=''.join(re.findall('(\w*)\.',output_file))

try:
	f_out = open(origName,'w')
	f_out.close()
	original.to_csv(origName,index=False)
except:
	print 'Error: Can not write to output file "%s"' % (origName)

try:
	f_out = open(dupName,'w')
	f_out.close()
	duplicates.to_csv(dupName,index=False)
except:
	print 'Error: Can not write to output file "%s"' % (dupName)
